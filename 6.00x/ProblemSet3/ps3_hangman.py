...
def hangman(secretWord):
    ...
    * After each round, you should also display to the user the 
    ...
    while  chancesLeft > 0:
        ...
        print "You have %d guesses left." %chancesLeft
        print "Available Letters: %s" %getAvailableLetters(lettersHistory)
        guess = raw_input("Please guess a letter: ").lower()
        
        if guess in lettersHistory:
            print "Oops! You've already guessed that letter: %s" %getGuessedWord(secretWord, lettersGuessed)
        else:
            lettersHistory.append(guess)
